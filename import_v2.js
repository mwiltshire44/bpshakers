...
async function runImport() {
  ...
  // Load existing categories from Firestore for tag matching
  const catSnap = await db.collection("categories").get();
  const categoryMap = {}; // { "Christmas Themed": "firestoreDocId", ... }
  catSnap.docs.forEach(d => { categoryMap[d.data().name] = d.id; });
  console.log(`Loaded ${Object.keys(categoryMap).length} categories: ${Object.keys(categoryMap).join(", ")}\n`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Resolve tags to category IDs — only keep exact matches
    const rawTags = (row["tags"] || "").trim();
    const categories = rawTags
      ? rawTags.split(",").map(t => t.trim()).filter(t => categoryMap[t] !== undefined).map(t => categoryMap[t])
      : [];
